...
export const ProgressBar: FC<IProgressBarProps> = (
  ...
  const WRAPPER_MAIN_PAGE_CLASSNAME = "relative h-[8px] w-full rounded-[32px] border border-black-300";
  const INNER_MAIN_PAGE_CLASSNAME = "absolute left-0 top-0 h-full bg-gradient-loading";

  const WRAPPER_STATS_PAGE_CLASSNAME = "relative h-[8px] w-[296px] rounded-[32px] border border-white-900";
  const INNER_STATS_PAGE_CLASSNAME = "absolute -bottom-[2px] -left-[2px] z-[10] h-[10px] w-[45%] rounded-[32px] border-none bg-gradient-button-accent";
  ...
  return (
    <div className={twMerge(classNames.wrapper)}>
      <div style={{ width: `${percentage}%` }}
           className={twMerge(classNames.inner)}
      />
    </div>
  );
};